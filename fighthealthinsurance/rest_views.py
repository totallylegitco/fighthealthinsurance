...
class AppealViewSet(viewsets.ViewSet):
    ...
    def send_fax(self, request) -> Response:
        ...
        if appeal.patient_user == patient_user and appeal.for_denial.professional_to_finish:
            ...
            staged = common_view_logic.SendFaxHelper.stage_appeal_as_fax(
                ...
                professional=True)
            common_view_logic.SendFaxHelper.remote_send_fax(uuid = staged.uuid, hashed_email=staged.hashed_email)
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    def assemble_appeal(self, request) -> Response:
        ...
        denial = Denial.filter_to_allowed_denials(current_user).get(denial_uuid=denial_uuid)
        ...
        pubmed_articles_to_include = serializer.validated_data["pubmed_articles_to_include"]
        include_provided_health_history = serializer.validated_data["include_provided_health_history"]
...
class SendToUserViewSet(viewsets.ViewSet, SerializerMixin):
    ...
    def post(self, request):
        ...
        appeal = Appeal.filter_to_allowed_appeals(current_user).get(id=serializer.validated_data["appeal_id"])