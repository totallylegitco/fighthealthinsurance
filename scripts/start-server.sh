...
export HOME=$(pwd)
gunicorn fighthealthinsurance.wsgi --user www-data --bind 0.0.0.0:8010 --workers 4 --access-logformat  "{\"actual_ip\": \"%({X-Forwarded-For}i)s\", \"remote_ip\":\"%(h)s\",\"request_id\":\"%({X-Request-Id}i)s\",\"response_code\":\"%(s)s\",\"request_method\":\"%(m)s\",\"request_path\":\"%(U)s\",\"request_querystring\":\"%(q)s\",\"request_timetaken\":\"%(D)s\",\"response_length\":\"%(B)s\", \"agent\": \"\%(a)s\"}" --error-logfile /tmp/gerror.log --access-logfile /tmp/gaccess.log 2>&1 | grep -v kube-probe  | grep -v kube-proxy &
sleep 2
