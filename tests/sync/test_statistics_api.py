    ...
    def test_get_statistics(self):
        with suppress_deprecation_warnings():
            try:
                ...
                print(f"Database counts - Total: {current_appeals.count()}, Pending: {current_pending}, Sent: {current_sent}")
                
                self.assertEqual(response.data['current_total_appeals'], 10,
                               f"Expected 10 total appeals, got {response.data['current_total_appeals']}")
                self.assertEqual(response.data['current_pending_appeals'], 5,
                               f"Expected 5 pending appeals, got {response.data['current_pending_appeals']}")
                self.assertEqual(response.data['current_sent_appeals'], 5,
